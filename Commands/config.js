...
exports.info = "Used to modify, create, or delete any server setting (with exceptions). Typing '(prefix)config display' will give you a list of existing configs and their values. Set, add, and remove are three different actions you can make. examples:\n*config set greet false\n*config add banned_users 234642241564442625\n*config remove banned_users 234642241564442625\nSetting a config to undefined will delete that config. It's possible to set arrays by formatting them as such: [item1,item2,item3]. Shorthand commands such as *disable and *enable will do the same thing as this command, but only modify certain configs. Users/channels are represented by their IDs in banned_users, admins, greet_channel, etc. Type '*config display readable' to replace these with the user/channel name. Setting/adding/removing a mention will automatically be turned into an ID.";
